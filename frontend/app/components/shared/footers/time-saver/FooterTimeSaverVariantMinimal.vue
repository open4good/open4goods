...
<template>
  <v-sheet
    class="time-saver time-saver--minimal"
    rounded="xl"
    border
    color="surface-default"
  >
    <div class="time-saver__row">
      <div class="time-saver__copy">
        <p class="time-saver__eyebrow text-subtitle-2 mb-1">
          {{ model.eyebrow }}
        </p>
        <h3 class="time-saver__title text-h6 mb-2">{{ model.title }}</h3>
        <p class="time-saver__subtitle text-body-2 mb-0">
          {{ model.subtitle }}
        </p>
      </div>
...
.time-saver
  ...
  border-color: rgba(var(--v-theme-border-primary-strong), 0.35)
