...
describe('_subset-to-filters helpers', () => {
  ...
  it('merges subset selections within the same group using OR semantics for term filters', () => {
    ...
    expect(request).toEqual({
      filterGroups: [
        {
          filterSets: [
            { filters: [{ field: 'price.conditions', operator: 'term', terms: ['NEW'] }] },
            { filters: [{ field: 'price.conditions', operator: 'term', terms: ['OCCASION'] }] },
          ],
        },
      ],
    })
  ...
  it('combines compatible range filters by widening the bounds', () => {
    ...
    expect(request.filterGroups?.[0]?.filterSets).toEqual([
      { filters: [expect.objectContaining({ field: 'price.min', operator: 'range', max: 800 })] },
      { filters: [expect.objectContaining({ field: 'price.min', operator: 'range', max: 500 })] },
    ])
  })
  ...
  it('drops conflicting range filters when they cannot be merged safely', () => {
    ...
    expect(request.filterGroups?.[0]?.filterSets).toEqual([
      { filters: [expect.objectContaining({ field: 'price.min', operator: 'range', max: 500 })] },
      { filters: [expect.objectContaining({ field: 'price.min', operator: 'range', min: 1000 })] },
    ])
  })
})
