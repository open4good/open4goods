...
test.describe('Opensource Page', () => {
  test('should match snapshot', async ({ page }) => {
    await expect(page).toHaveScreenshot('opensource-page.png', {
      fullPage: true,
    })
  })
  ...
  test('should load without console errors', async ({ page }) => {
    ...
    const validErrors = consoleLogs.filter(
      log =>
  ...
  test('should display primary sections', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Un écosystème ouvert et fiable' })
    ).toBeVisible()
    await expect(
      page.getByRole('heading', {
        name: 'Rejoignez le collectif en quelques étapes',
      })
    ).toBeVisible()
    await expect(
      page.getByRole('heading', { name: 'Tout pour bien démarrer' })
    ).toBeVisible()
  })
