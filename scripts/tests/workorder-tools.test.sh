...
printf '%s\n' \
  ...
  'spec:' \
  '  execution:' \
  '    promotionAuthorizationRef: owner-order-2026-09-17-local-first-promotion' \
  '  milestones:' \
...
write_order('beta', phase='BETA_VALIDATION', priority=0)
write_order('prod', phase='PRODUCTION', priority=0, dependencies=['beta'])
...
assert next(entry for entry in all_phases if entry['id'] == 'beta')['availability'] == 'AWAITING_DEVELOPMENT'
assert next(entry for entry in all_phases if entry['id'] == 'prod')['availability'] == 'AWAITING_BETA_VALIDATION'
...
# Remove the phase-gate fixtures after their individual behavior has been verified.
for path in list(orders.glob('*.yml')):
    payload = yaml.safe_load(path.read_text())
    if payload['spec']['executionPhase'] == 'DEVELOPMENT':
        path.unlink()
assert yaml.safe_load(cli('status', 'beta').stdout)['availability'] == 'READY'
cli('begin', 'beta')
cli('close', 'beta', '--evidence', 'test:beta')
assert 'owner-order:owner-order-2026-09-17-local-first-promotion' in (orders / 'ledger/beta.yml').read_text()
assert yaml.safe_load(cli('status', 'prod').stdout)['availability'] == 'READY'
cli('begin', 'prod')
cli('close', 'prod', '--evidence', 'test:prod')
assert 'owner-order:owner-order-2026-09-17-local-first-promotion' in (orders / 'ledger/prod.yml').read_text()
...
write_order('readiness')
write_order('readiness-peer')
readiness_path = orders / 'readiness.yml'
readiness = yaml.safe_load(readiness_path.read_text())
readiness['spec']['completionGates'] = ['ALL_OTHER_DEVELOPMENT_COMPLETED']
readiness['spec']['requiredEvidence'] = ['local-full-recette:passed']
readiness_path.write_text(yaml.safe_dump(readiness))
assert 'all-other-development-workorders' in yaml.safe_load(cli('status', 'readiness').stdout)['blockers'][-1]
(orders / 'readiness.yml').unlink()
(orders / 'readiness-peer.yml').unlink()
...
assert 'Awaiting phase' in roadmap
assert 'The permanent promotion authority is recorded' in roadmap
