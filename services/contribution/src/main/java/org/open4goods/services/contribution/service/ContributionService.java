...
public class ContributionService implements HealthIndicator {
    ...
    public ContributionService(CacheManager cacheManager, SerialisationService serialisationService,
            ContributionVoteRepository repository, ReversementConfig reversementConfig,
    ...
    private Map<String, Long> byVotesSince(Long epoch) {
        ...
        esQuery = esQuery.withAggregation("vote", Aggregation.of(a -> a.terms(ta -> ta.field("vote").size(500))));

        SearchHits<ContributionVote> results = elasticSearchOperations.search(esQuery.build(), ContributionVote.class,
                IndexCoordinates.of(ContributionVote.INDEX_NAME));
