---
title: "Node Operator Agreement"
description: "*Last updated: 2026-05-04*"
tags:
  - documentation
  - vue-content
  - structural
  - frontend
owner: platform
audience: all
language: en
component: frontend
maturity: draft
security_classification: public
doc_url: /docs/apps/frontend/content/en/docs/legal/operator
doc_path: apps/frontend/content/en/docs/legal/operator.md
rag_chunking: heading
metadata_last_synced: 2026-05-08
---
# Node Operator Agreement

*Last updated: 2026-05-04*

Welcome to Product Data API. By enrolling your node, you agree to the following terms:

## 1. Node Responsibility
As an operator, you are responsible for the availability and security of your hardware.

## 2. Resource Allocation
You grant Product Data API the right to route inference tasks to your hardware based on the tiers you participate in.

## 3. Privacy & Security
Private keys generated on your node must remain local. You agree not to bypass mTLS security controls.

## 4. Compliance
You agree to comply with all local regulations regarding compute and data processing.
