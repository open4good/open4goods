...
import { buildSampleResponse, type FacetDescriptor } from '~/domains/b2b/facets'
...
const props = defineProps<{ facet: FacetDescriptor }>()
...
const gtin = ref(props.facet.examples[0]?.gtin ?? '')
...
const resolvedExamples = computed(() =>
  props.facet.examples.map(ex => ({ ...ex, label: t(ex.labelKey) }))
)
...
async function run () {
  ...
  try {
    if (mode.value === 'sample') {
      ...
      result.value = buildSampleResponse(props.facet, key) as PlaygroundResponse
    } else {
      result.value = await post<PlaygroundResponse>(props.facet.playgroundEndpoint, {
